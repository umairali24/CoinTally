...
class NotificationsHandlerService: MethodChannel.MethodCallHandler, NotificationListenerService() {
    ...
    private fun sendEvent(evt: NotificationEvent) {
        ...
        if (callbackHandle == 0L) {
            ...
            Log.d(TAG, "sendEvent: Looked up callbackHandle from SharedPreferences: $callbackHandle")
        }

        Log.d(TAG, "sendEvent: final callbackHandle=$callbackHandle, mBackgroundChannel initialized=${::mBackgroundChannel.isInitialized}")
        
        if (!::mBackgroundChannel.isInitialized) {
            Log.e(TAG, "sendEvent: mBackgroundChannel is NOT initialized! Attempting to updateFlutterEngine...")
        ...
        try {
            ...
            Log.d(TAG, "sendEvent: Successfully invoked sink_event")
        } catch (e: Exception) {
            Log.e(TAG, "sendEvent: Error invoking sink_event", e)
            e.printStackTrace()
        }
    }
